 ...
 * each slot renders an on-brand botanical gradient PLATE (sand / sage /
 * terracotta / night) that reads as one cohesive, grounded wellness shoot, plus
 * an explicit "sample" chip so the prospect understands these are placeholders
 * for their real imagery.
 *
 * Craft baked in:
 *  - CSS `aspect-ratio` → zero CLS, no width/height race
 *  - rounded corners + warm hairline border consistent with the system
...
export type BrandImageProps = {
  ...
  alt: string;
  aspect: string;
  /** plate tone — the cohesive botanical grade. */
  tone?: "sand" | "sage" | "terra" | "night";
...
const TONE: Record<NonNullable<BrandImageProps["tone"]>, string> = {
  sand: "km-plate",
  sage: "km-plate km-plate--sage",
  terra: "km-plate km-plate--terra",
  night: "km-plate km-plate--night",
};
...
export function BrandImage({
  ...
  return (
    ...
    >
      ...
      {scrim !== "none" && (
        <span
          ...
          className={cn(
            ...
            scrim === "soft"
              ? "bg-gradient-to-t from-[oklch(24%_0.03_130_/_0.6)] via-[oklch(24%_0.03_130_/_0.12)] to-transparent"
              : "bg-gradient-to-t from-[oklch(22%_0.03_130_/_0.84)] via-[oklch(24%_0.03_130_/_0.36)] to-[oklch(26%_0.03_130_/_0.08)]",
          )}
        />
      )}
      ...
      {sample && (
        <span className="pointer-events-none absolute right-3 top-3 z-10 rounded-full border border-[oklch(100%_0_0_/_0.4)] bg-[oklch(24%_0.03_130_/_0.5)] px-2.5 py-1 text-[0.6rem] font-semibold uppercase tracking-[0.2em] text-[oklch(96%_0.02_120)] backdrop-blur-sm">
          Sample
        </span>
      )}
