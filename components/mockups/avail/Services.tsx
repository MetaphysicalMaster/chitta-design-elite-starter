...
/**
 * Services — the real med-spa menu, organized as an engineered category grid.
 * Injectables · Lasers & Energy · Body · Skin & Facials · Wellness. Each card
 * lists real treatments and is book-able. Hover lifts + cobalt edge tick.
 */
...
const CATEGORIES: Category[] = [
  {
    id: "injectables",
    name: "Injectables",
    blurb: "Wrinkle relaxers & dermal fillers, mapped to your anatomy.",
    treatments: ["Botox & Dysport", "Juvéderm & RHA fillers", "Sculptra", "Kybella", "Lip enhancement"],
  ...
  },
  {
    ...
    treatments: ["Laser hair removal", "IPL photofacial", "Morpheus8 RF microneedling", "Laser resurfacing"],
  ...
  },
  {
    ...
    treatments: ["CoolSculpting", "EmSculpt NEO", "Skin tightening", "Cellulite reduction"],
  ...
  },
  {
    ...
    treatments: ["IV vitamin therapy", "Hormone optimization", "Medical weight loss", "B12 & NAD+"],
...
function Card({ c, index }: { c: Category; index: number }) {
  return (
    <Reveal delay={index * 0.06} className={c.span ? "sm:col-span-2 xl:col-span-1 xl:row-span-2" : ""}>
      <article
        className={cn(
          "group relative flex h-full flex-col overflow-hidden rounded-[1.4rem] border border-[var(--color-border)] bg-[var(--color-bg-elevated)] p-6",
          "shadow-[var(--glass-shadow)] transition-[transform,box-shadow] duration-300 hover:-translate-y-1 hover:border-[var(--color-accent)]/40",
        )}
      >
        {/* cobalt edge tick that grows on hover */}
        <span
          aria-hidden
          className="absolute left-0 top-6 h-8 w-1 origin-top rounded-r bg-[var(--color-accent)] opacity-0 transition-opacity duration-300 group-hover:opacity-100"
        />
    ...
    >
      <div className="mx-auto max-w-7xl px-6 sm:px-8">
        <SectionHeading
          ...
          title={
            <>
              Every treatment. <span className="text-[var(--color-accent-deep)]">Same standard</span>, every location.
            </>
          }
          lead="A complete medical aesthetics menu — injectables, energy-based devices, body contouring, advanced skin and wellness — delivered to one protocol across all four homes."
        />
