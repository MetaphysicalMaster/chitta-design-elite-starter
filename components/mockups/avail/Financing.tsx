...
/**
 * Financing — CareCredit / financing callout. Removes the price objection with
 * an engineered, confident band: pay-over-time, transparent, every location.
 */
...
const POINTS = [
  ...
  "Apply in minutes — decision before your appointment",
  "Accepted at all four Avail Aesthetics locations",
];
      ...
      <div className="mx-auto max-w-7xl px-6 sm:px-8">
        <Reveal>
          <div className="relative overflow-hidden rounded-[1.75rem] border border-[var(--color-accent)]/30 bg-[var(--color-bg-elevated)] p-8 shadow-[var(--glass-shadow)] sm:p-10 lg:p-12">
            {/* cobalt slipstream accent corner */}
            <div
              aria-hidden
            ...
            <div className="relative grid grid-cols-1 gap-8 lg:grid-cols-[1.2fr_1fr] lg:items-center">
              <div>
                <p className="rule-cobalt inline-block text-xs font-semibold uppercase tracking-[0.24em] text-accent-deep">
                  Financing
                </p>
                <h2
                ...
                <p className="mt-4 max-w-lg text-pretty text-[var(--color-fg-muted)]" style={{ fontSize: "var(--fluid-lead)" }}>
                  Flexible financing through <span className="font-semibold text-[var(--color-fg)]">CareCredit</span> and
                  Cherry — so the right treatment plan is never out of reach.
                </p>
                <ul className="mt-6 flex flex-col gap-3">
                  {POINTS.map((p) => (
                    <li key={p} className="flex items-start gap-3 text-sm text-[var(--color-fg)]">
                      <span aria-hidden className="mt-0.5 grid h-5 w-5 shrink-0 place-items-center rounded-full bg-[var(--color-accent)] text-[var(--color-accent-fg)]">
              ...
              <div className="flex flex-col gap-3 rounded-2xl border border-[var(--color-border)] bg-[var(--color-bg-subtle)] p-6">
                ...
                <p className="font-display text-3xl font-semibold tnum text-[var(--color-fg)]">
                  $89<span className="text-base font-medium text-[var(--color-fg-muted)]">/mo</span>
                </p>
                <p className="text-xs text-[var(--color-fg-subtle)]">
                  e.g. $1,068 plan · 12 months · subject to credit approval. Sample figure.
                </p>
                <Link
                  href="#book"
                  className={cn(
