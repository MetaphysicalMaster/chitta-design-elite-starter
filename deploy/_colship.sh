...
for entry in "${CLIENTS[@]}"; do
  ...
  ( cd "$dest"; rm -rf .git; git init -q -b main; git add -A
    git -c user.email=noreply@anthropic.com -c user.name=Claude commit -q -m "Deploy ${slug}: national-7-standard elevation (signature hero + concierge + reviews + before/after)"
  ...
  echo "    -> https://metaphysicalmaster.github.io/${slug}/"
done
rm -rf out; rm -f deploy/_b_*.log
echo "COLUMBUS REDEPLOY DONE"
